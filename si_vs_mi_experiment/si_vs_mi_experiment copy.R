...
results <- data.frame(p = integer(),
                      ...
                      shd = integer(),
                      number_of_edges_estimated_imp = integer(),
                      number_of_edges_estimated_full = integer(),
                      stringsAsFactors = FALSE)

filename <- "results_edgecount_config2n3000.csv"
...
for (i in 1:99){  
  ...
  for (m in c(1, 10, 100, 1000)) {
    ...
    if (m == 1000){
      ...
      print(m)
      suff_imputed <- suffStatMI_full[-(1:(m_max - m))]
    }
    else {
      print(m)
    ...
    #pc_complete <- cu_pc_MI(suffStat = suff_complete, indepTest = pcalg::gaussCItest, alpha = alpha, p = p)
    pc_complete <- cu_pc_MI(suffStat = suff_complete, alpha = alpha, p = p, df_method = "br")
    adjmat_complete <- as(pc_complete@graph, "matrix")
    number_of_edges_estimated_full <- ecount(graph_from_graphnel(pc_complete@graph))
    # Run PC algorithm on imputed data with each df_correction_method
    if (m > 1){
      for (df_correction_method in c('br')) {
        pc_imputed <- cu_pc_MI(suffStat = suff_imputed, alpha = alpha, p = p, df_method = 'br')
        mean_lambda <- mean(lambdas)
        mean_df <- mean(dfs)
        lambdas <- c()
        dfs <- c()
        number_of_edges_estimated_imp <- ecount(graph_from_graphnel(pc_imputed@graph))
        ...
        results <- rbind(results, data.frame(p = p,
                                             ...
                                             number_of_edges_estimated_imp = number_of_edges_estimated_imp,
                                             number_of_edges_estimated_full = number_of_edges_estimated_full,
                                             stringsAsFactors = FALSE))
    ...
    # m = 1
      ...
      pc_imputed <- cu_pc_MI(suffStat = suff_imputed, alpha = alpha, p = p, df_method = 'old')
...
results %>%
  ...
  summarise(mean_recall = mean(recall, na.rm = TRUE),
            ...
            edges_full = mean(number_of_edges_estimated_full),
            edges_imp = mean(number_of_edges_estimated_imp)) %>% View()
...
write.csv(results, "results_edgecount_config2n3000.csv")
