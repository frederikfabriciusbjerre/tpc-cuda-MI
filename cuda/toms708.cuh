...
#define M_SQRT_PI 1.77245385090551602729
#define M_1_SQRT_2PI 0.39894228040143267794
#define num_IT 20

#define min(a,b) ((a)<(b)?(a):(b))
#define max(a,b) ((a)>(b)?(a):(b))
// from pgamma.c in r-source
__device__ static double logspace_add(double logx, double logy) {
    return max(logx, logy) + log1p (exp (-fabs (logx - logy)));
}

// Helper functions declarations
__device__ static double exparg(int l);
__device__ static double fpser(double a, double b, double x, double eps, int log_p);
__device__ static double bpser(double a, double b, double x, double eps, int log_p);
__device__ static double apser(double a, double b, double x, double eps);
__device__ static double bup(double a, double b, double x, double y, int n, double eps, int give_log);
__device__ static double bfrac(double a, double b, double x, double y, double lambda, double eps, int log_p);
__device__ static void bgrat(double a, double b, double x, double y, double *w, double eps, int *ierr, int log_w);
__device__ static double basym(double a, double b, double lambda, double eps, int log_p);
__device__ static double brcomp(double a, double b, double x, double y, int log_p);
__device__ static double brcmp1(int mu, double a, double b, double x, double y, int give_log);
__device__ static double grat_r(double a, double x, double log_r, double eps);
__device__ static double esum(int mu, double x, int give_log);
__device__ static double alnrel(double a);
__device__ static double rlog1(double x);
__device__ static double erf__(double x);
__device__ static double erfc1(int ind, double x);
__device__ static double gam1(double a);
__device__ static double gamln1(double a);
__device__ static double psi(double x);
__device__ static double betaln(double a0, double b0);
__device__ static double algdiv(double a, double b);
__device__ static double gsumln(double a, double b);
__device__ static double bcorr(double a0, double b0);
__device__ static double gamln(double a);
__device__ static double rexpm1(double x);
__device__ static double Rf_d1mach(int i);
__device__ static int Rf_i1mach(int i);

// Main function
__device__ void bratio(double a, double b, double x, double y, double *w, double *w1,
                       int *ierr, int log_p)
{
    // Local variables
    bool do_swap;
    ...
    bool a_lt_b = (a < b);
    // Machine epsilon
    double eps = 2.2204460492503131e-16; // 2 * DBL_EPSILON
    ...
    *w1 = 0.0;
    ...
    // Input validation
    if (isnan(x) || isnan(y) || isnan(a) || isnan(b)) { *ierr = 9; return; }
    ...
    *ierr = 0;
    ...
    if (min(a,b) <= 1.0) { /*------------------------ a <= 1  or  b <= 1 ---- */
        ...
        n = 20; /* goto L130; */
        *w1 = bup(b0, a0, y0, x0, n, eps, 0); did_bup = true;
        b0 += n;
    L131:
        bgrat(b0, a0, y0, x0, w1, 15*eps, &ierr1, 0);
        if(*w1 == 0 || (0 < *w1 && *w1 < DBL_MIN)) {
            if(did_bup) {
                *w1 = bup(b0-n, a0, y0, x0, n, eps, 1); // log_p = 1
            }
            else *w1 = -INFINITY; // = 0 on log-scale
            bgrat(b0, a0, y0, x0, w1, 15*eps, &ierr1, 1); // log_p = 1
...
L140:
    ...
    *w = bup(b0, a0, y0, x0, n, eps, 0); // log_p = 0
    ...
    if (x0 <= 0.7) {
        ...
        *w += bpser(a0, b0, x0, eps, 0); // log_p = 0
        goto L_end_from_w;
    }
    /* L150: */
    if (a0 <= 15.0) {
        n = 20;
        *w += bup(a0, b0, x0, y0, n, eps, 0); // log_p = 0
        a0 += n;
    }
    bgrat(a0, b0, x0, y0, w, 15.0*eps, &ierr1, 0); // log_p = 0
    if(ierr1) *ierr = 10 + ierr1;
    goto L_end_from_w;
...
__device__ static double bup(double a, double b, double x, double y, int n, double eps,
		  int give_log)
{
/* ----------------------------------------------------------------------- */
...
// Obtain the scaling factor exp(-mu) and exp(mu)*(x^a * y^b / beta(a,b))/a
    ...
    if (n == 1 ||
	(give_log && ret_val == -INFINITY) || (!give_log && ret_val == 0.))
	return ret_val;
...
__device__ static double bfrac(double a, double b, double x, double y, double lambda,
...
/* -----------------------------------------------------------------------
   ...
   -----------------------------------------------------------------------*/
    ...
    if(isinf(lambda)) return (0.0 / 0.0);// TODO: can return 0 or 1 (?)
    brc = brcomp(a, b, x, y, log_p);
    if(isnan(brc)) { // e.g. from   L <- 1e308; pnbinom(L, L, mu = 5)
	return (0.0 / 0.0); // TODO: could we know better? NAN
    }
    if (!log_p && brc == 0.) {
	return 0.;
    }
...
/* ----------------------------------------------------------------------- */
    ...
    // constant inserted is equivalent to M_LN_SQRT_2PI
	return(log_p
	       ? -0.918938533204672741780329736406 + .5*log(b * x0) + z - bcorr(a,b)
...
__device__ static void bgrat(double a, double b, double x, double y, double *w,
...
/*                    ADD THE RESULTS TO W */
	...
	*w += ((fabs(u) < 1e-12) ? exp(log_u + log(sum)) : u * sum);
    return;
} /* bgrat */
...
/*            THE ARRAYS A0, B0, C, D HAVE DIMENSION NUM + 1. */
    static const double e0 = 1.12837916709551;/* e0 == 2/sqrt(pi) */
    static const double e1 = .353553390593274;/* e1 == 2^(-3/2)   */
    static const double ln_e0 = 0.120782237635245; /* == ln(e0) */
...
__device__ static double Rf_d1mach(int i)
{
    ...
    case 5: return 0.301029995663981195213738894724; // M_LOG10_2

    default: return 0.0;
    }
}

__device__ int Rf_i1mach(int i)
{
    switch(i) {
